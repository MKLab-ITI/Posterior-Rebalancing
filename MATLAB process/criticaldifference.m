function [cd] = criticaldifference(s,labels,alpha,fig_title,pvalue)
%
...
axis([-0.5 1.5 0 140]);
axis xy 
title(fig_title,'Color','blue')
 
tics = repmat((0:(k-1))/(k-1), 3, 1);
line(tics(:), repmat([100, 104, 100], 1, k), 'LineWidth', 1, 'Color', 'k');
tics = repmat(((0:(k-2))/(k-1)) + 0.5/(k-1), 3, 1);
line(tics(:), repmat([100, 102, 100], 1, k-1), 'LineWidth', 1, 'Color', 'k');
line([0 0 0 cd/(k-1) cd/(k-1) cd/(k-1)], [120 116 118 118 116 120], 'LineWidth', 1, 'Color', 'k');
h = text(0.5*cd/(k-1) + 0.3, 128, strcat('Friedman p=', num2str(pvalue)), 'FontSize', 17);
h = text(0.5*cd/(k-1) + 0.3, 120, strcat('Nemenyi  p=', num2str(alpha)), 'FontSize', 17);
h = text(0.5*cd/(k-1), 128, strcat('CD = ',sprintf('%5.2f',cd)), 'FontSize', 17, 'HorizontalAlignment', 'center');
 
%tics = repmat((0:(k-1))/(k-1), 3, 1);
%repmat([100, 105, 100], 1, k);
...
%h = text(0.5*cd/(k-1), 130, strcat('CD = ',sprintf('%5.2f',cd)), 'FontSize', 12, 'HorizontalAlignment', 'center');
 
for i=1:k
 
   text((i-1)/(k-1), 108, num2str(k-i+1), 'FontSize', 17, 'HorizontalAlignment', 'center');
...
% for i=1:ceil(k/2)
% 
%    line([(k-r(i))/(k-1) (k-r(i))/(k-1) 1.1], [100 112-4*(n+1)-8*i 112-4*(n+1)-8*i], 'Color', 'k');
%    h = text(1.23, 116 - 5.5*(n+1)- 8*i, sprintf('%5.2f',r(i)), 'FontSize', 8, 'HorizontalAlignment', 'right');
% 
%    text(1.26, 116 - 5.5*(n+1) - 8*i, labels{idx(i)}, 'FontSize', 9, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
% 
% end
% 
...
% for i=ceil(k/2)+1:k
% 
%    line([(k-r(i))/(k-1) (k-r(i))/(k-1) -0.1], [100 112-4*(n+1)-8*(k-i+1) 112-4*(n+1)-8*(k-i+1)], 'Color', 'k');
% 
%    text(-0.24, 116 - 5.5*(n+1) -8*(k-i+1), sprintf('%5.2f',r(i)), 'FontSize', 8, 'HorizontalAlignment', 'left');
% 
%    text(-0.26, 116 - 5.5*(n+1) -8*(k-i+1), labels{idx(i)}, 'FontSize', 9, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
% 
% end
% 
...
%    R = r(clique(i,:));
% 
%    line([((k-min(R))/(k-1)) + 0.005 ((k - max(R))/(k-1)) - 0.005], [100-3*i 100-3*i], 'LineWidth', 2, 'Color', 'k');
% 
% end
...
for i=1:ceil(k/2)
  ...
  %text(1.2, 101 - 5*(n+1)- 10*i + 2, sprintf('%5.3f',r(i)), 'FontSize', 17, 'HorizontalAlignment', 'right');
  %text(1.23, 101 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 17, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'Rotation', 30);
  text(1.2, 102 - 5*(n+1)- 10*i + 2, labels{idx(i)}, 'FontSize', 16, 'HorizontalAlignment', 'right');
  text(1.17, 100 - 5*(n+1) - 10*i, sprintf('%5.2f',r(i)), 'FontSize', 17, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
for i=ceil(k/2)+1:k
  ...
  text(-0.2, 102 - 5*(n+1) -10*(k-i+1)+2, labels{idx(i)}, 'FontSize', 16, 'HorizontalAlignment', 'left');
  text(-0.2, 100 - 5*(n+1) -10*(k-i+1), sprintf('%5.2f',r(i)), 'FontSize', 17, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
...
for i=1:size(clique,1)
  ...
  R = r(clique(i,:));
  line([((k-min(R))/(k-1)) + 0.005 ((k - max(R))/(k-1)) - 0.005], [100-5*i 100-5*i], 'LineWidth', 3, 'Color', 'k');
